...
using SASedWarp.Unity.Runtime;
using UitkForKsp2.API;
using UnityEngine;
using UnityEngine.UIElements;

namespace SASedWarp.UI;
...
public class MyFirstWindowController : MonoBehaviour
{
    ...
    public bool IsWindowOpen
    {
        ...
        set
        {
            ...
            GameObject.Find(SASedWarpPlugin.ToolbarFlightButtonID)
                ?.GetComponent<UIValue_WriteBool_Toggle>()
                ?.SetValue(value);

            // Update the OAB AppBar button state
            GameObject.Find(SASedWarpPlugin.ToolbarOabButtonID)
                ?.GetComponent<UIValue_WriteBool_Toggle>()
                ?.SetValue(value);
        }
    }
